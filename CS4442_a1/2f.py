...
# -------------------- Part f) where we are repeating b) -------------------------
...
# Creating our matrix for x with column of 1's, column of x^2's , column of x^3's and column of x^4's
temp = np.ones((len(x), 5))
temp[:,0] = np.power(np.asarray(x), 4)
temp[:,1] = np.power(np.asarray(x), 3)
temp[:,2] = np.power(np.asarray(x), 2)
temp[:,3] = np.asarray(x)
x = temp
y = np.asarray(y)
...
h = w_0 * x4 + w_1 * x3 + w_2 * x2 + w_3 * x1 + w_4

# We will use poly1d() function for plotting instead of using our h,
...
plot1 = plt.figure(1)
plt.title("4th-Order Polynomial Regression and Training Data", color="black")
...
# -------------------- Part f) where we are repeating c) -------------------------
...
h_test = w_0 * x4_test + w_1 * x3_test + w_2 * x2_test + w_3 * x1_test + w_4

# Finally plotting our regression model and test data
plot2 = plt.figure(2)
plt.title("4th-Order Polynomial Regression and Testing Data", color="black")
